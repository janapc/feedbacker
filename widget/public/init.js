function init (apiKey) {
  async function handleLoadWidget () {
    ...
    const WIDGET_URL = `https://janapc-feeadbacker-widget.netlify.app?api_key=${apiKey}&page=${page}&fingerprint=${fingerprint.visitorID}`;

    const config = { method: 'HEAD' };
    const response = await fetch(
      `https://backend-vue3js-fwdo2n3r5-janapc.vercel.app/apikey/exists?apikey=${apiKey}`,
      config
    );
